...
def log_results(*args):
    ...
    with open(f"{log_dir}results{datetime.now().strftime('%m%d%H%M')}.txt", "w") as f:
        ...
        print_(
            f"Model trained for {epochs} epochs and {patience} patience. Below scores mean the best validation losses achieved.\n")
...
for arg_list in tqdm(arg_values):
    ...
    descr += "\n"
    print(descr)
    run_id, score = run(dataset_and_model, 3, create_network, experiment_kwargs, epochs=epochs, patience=patience,
                        descr=descr)
    time.sleep(1)
